    ...
    <header class="p-5 border-b bg-white shadow">
        <div class="container mx-auto flex justify-between items-center">
            ...
            <nav class="flex gap-2 items-center">
                ...
                <a class="font-bold text-sm" href="{{route('posts.index',auth()->user()->username)}}">
                    Hola:
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="font-bold uppercase text-sm">Cerrar Sesión</button>
                </form>
            </nav>
            @endauth
            {{-- verificacion usuario invitado --}}
            @guest
            {{-- barra navegación --}}
            <nav class="flex gap-2 items-center">
            ...
            @endguest
            {{-- fin verificación autenticación --}}
        </div>
    </header>
